...
BOOL CMapiSession::Resolve(const CString& sName, lpMapiRecipDesc* lppRecip)
{
  ...
  // modified by Will.T
  // do not resolve the name if user is not logged on
  if (!LoggedOn())
  {
	  return FALSE;
  }
...
// modifed: add a param for parent window
BOOL CMapiSession::Send(CMapiMessage& message, HWND hWndParent)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	USES_CONVERSION;
  ...
  ULONG nError = m_lpfnMAPISendMail(m_hSession, (ULONG_PTR)hWndParent, &mapiMessage, MAPI_LOGON_UI|MAPI_DIALOG, 0);
  if (nError == SUCCESS_SUCCESS)
  {
    bSuccess = TRUE;
    m_nLastError = SUCCESS_SUCCESS;
